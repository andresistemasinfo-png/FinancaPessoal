...
export const Layout: React.FC = () => {
  return (
    <div className="flex h-screen bg-emerald-50/30 text-emerald-950 font-sans">
      ...
      {/* Bottom Nav (Mobile) */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-emerald-100 flex justify-around p-2 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-50">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
